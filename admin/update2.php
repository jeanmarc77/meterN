<?php
/**
 * /srv/http/metern/admin/update2.php
 *
 * @package default
 */
...
if (!file_exists("$SRVDIR/_INSTALL/")) {
	if (!mkdir("$SRVDIR/_INSTALL/", 0777, true) || $CURDIR == $SRVDIR) {
		$error = true;
		echo "<img src='../images/24/sign-error.png' width=24 height=24> Can't create $SRVDIR/_INSTALL. Make sure you didn't install meterN on web server's root directory !<br>";
	}
}
...
if (!$error) {
	if (file_exists("$SRVDIR/_INSTALL/metern/")) {
		rmdir("$SRVDIR/_INSTALL/metern/");
	}
