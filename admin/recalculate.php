<?php
/**
 * /srv/http/metern/admin/recalculate.php
 *
 * @package default
 */
...
for ($j = 1; $j <= $daythatm; $j++) {
	...
	echo "<tr align='center'><td>$dday</td><td>$val_day[$j]</td><td><input type='number' min=0 value='$val_day_calc[$j]' id='new$j' name='new$j' style='width:100px' step=$step onchange=\"recalculateSum();\"/ ";
	if ($stamp >= $today) {
		echo 'disabled';
	}
	echo '></td></tr>';
}
echo "<tr valign='top' align='center'><td></td><td><b>&Sigma; $sum ${'UNIT' . $metnum}</b></td><td><b>&Sigma; <span id='nsum'>$sum2</span> ${'UNIT' . $metnum}</b><br>(&#x394; <span id='diff'>0</span>${'UNIT' . $metnum})</td></tr>
</table>
<input id='sum' type='hidden' value=$sum2>
<input id='preci' type='hidden' value=${'PRECI' . $metnum}>
<br>
</fieldset>
<br><br>
<INPUT TYPE='button' onClick=\"location.href='admin.php'\" value='Cancel'> <input type='submit' id='bntsubmit' name='bntsubmit' onclick=\"return confirm('Are you sure ?')\" value='Continue'>
